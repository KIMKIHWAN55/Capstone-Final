...
def calculate_total_fitness_optimized(
    ...
    for i in range(n_expected):
        ...
        # ti_iloc가 배열 길이를 벗어나거나, 찾은 위치의 값이 ti_index와 다르면
        # -> T_i가 data에 없는 것임 (예: 휴장일). 이 T_i는 무시.
        ...
        # 페널티 범위를 위한 정수 위치
        # T_{i-1}과 T_{i+1}은 data에 없을 수도 있으므로, 
        # searchsorted가 반환하는 '근사' 위치를 그대로 사용 (이것이 견고한 방식)
        ti_prev_iloc = np.searchsorted(data_indices_arr, ti_prev_index, side='left')
        
        # T_{i+1}의 iloc를 찾을 때, NumPy 슬라이싱(.iloc)은 끝을 포함하지 않으므로
        # +1을 해줘야 .loc[a:b] (b포함)와 동일한 효과를 냄.
        # searchsorted는 기본적으로 side='left'이므로 T_{i+1} 당일 값을 찾음.
        ti_next_iloc = np.searchsorted(data_indices_arr, ti_next_index, side='left')
        # +1을 하여 해당 인덱스를 포함하도록 함.
        # 단, 배열 경계를 넘지 않도록 min() 처리
