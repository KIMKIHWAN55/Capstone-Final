...
# [신규] 파라미터 경계값을 전역 변수로 정의 (데코레이터가 사용)
PARAM_BOUNDS = {
...
# [신규] 개체(Individual) 순서에 맞는 바운드 리스트
# (min, max, type) 튜플
BOUNDS_LIST = [
    (*PARAM_BOUNDS['N'], int),
    (*PARAM_BOUNDS['n'], int),
...
# [신규] 파라미터 범위 및 제약조건(N > n)을 강제하는 데코레이터
def create_bounds_decorator(bounds_list):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            # 1. 원본 변이/교배 함수를 실행 (e.g., mutGaussian)
            offspring = func(*args, **kwargs)
            
            # 2. 반환된 모든 개체(자손)를 순회
            for ind in offspring:
                # 3. 각 파라미터 값을 기본 범위(Bounds) 내로 강제
                ...
                # 4. 특수 제약조건(Constraint) 강제: N > n
                # ind[0] == N, ind[1] == n
                if ind[0] <= ind[1]:
                    # N과 n이 범위 내에 있다는 것이 보장된 상태
                    # N을 n+1로 설정 (가장 간단한 "수리")
                    ind[0] = ind[1] + 1
                    
                    # 만약 N이 n+1이 되면서 최대치를 넘으면,
                    # 대신 n을 N-1로 설정
                    if ind[0] > bounds_list[0][1]: # N이 N_max(150)를 넘으면
                        ind[0] = bounds_list[0][1]   # N = 150
                        ind[1] = ind[0] - 1          # n = 149
...
# --- generate_MA_signals_vectorized (변경 없음) ---
# (내부에 N <= n 대비 로직이 이미 있으므로 그대로 둡니다)
...
# [수정] evaluate_MA_individual: 파라미터 클리핑(clipping) 로직 모두 제거
def evaluate_MA_individual(
    individual, df_data, expected_trading_points_df
    # , param_bounds # [삭제]
):
    """
    데코레이터가 파라미터 범위와 N > n을 보장하므로,
    이 함수 내의 np.clip 및 N <= n 검사 로직을 모두 제거합니다.
    """
    
    # 8개 파라미터 언패킹 (이미 유효한 값)
...
# [수정] run_MA_ga_optimization: 병렬 처리 로직 제거 및 데코레이터 적용
def run_MA_ga_optimization(
    df_input: pd.DataFrame, 
    generations: int = 50, 
    population_size: int = 50, 
    seed: int = None
    # , use_parallel: bool = True # [삭제]
):
    ...
    # [삭제] PARAM_BOUNDS 딕셔너리를 여기서 정의할 필요 없음 (전역으로 이동)
