...
from ESN_Signals5 import esn_signals
import warnings
import multiprocessing
import talib

import CPM2 as cpm
import MovingAverage3 as ma
import RSI3 as rsi
import ROC3 as roc
import importlib
...
# --- [신규] 워밍업을 위한 최대 룩백(Lookback) 기간 정의 ---
# 각 TA 모듈에 정의된 PARAM_BOUNDS에서 최대값을 가져옵니다.
try:
    # 예: ma.PARAM_BOUNDS['N'][1] (150)
    MAX_LOOKBACK_MA = ma.PARAM_BOUNDS['N'][1] 
    # 예: rsi.PARAM_BOUNDS['x'][1] (30)
    MAX_LOOKBACK_RSI = rsi.PARAM_BOUNDS['x'][1] 
    # 예: roc.PARAM_BOUNDS['p_long'][1] (100)
    ...
    print(f"--- [워밍업 설정 오류] ---")
    print(f"    {e}")
    print(f"    TA 모듈(ma, rsi, roc)에 'PARAM_BOUNDS' 딕셔너리나 관련 키가 없습니다.")
    print(f"    최대 룩백을 200으로 임의 설정합니다. (N=150 가정)")
    print(f"--------------------------")
    # ma.PARAM_BOUNDS['N'][1] (150)이 가장 클 것이라 가정하고 안전한 대체값(200) 사용
...
def fitness_function_with_backtesting(params, train_df: pd.DataFrame, test_df: pd.DataFrame, Technical_Signals=None):
    ...
    try:
        ...
        # fitness = stats['Sortino Ratio']
        # fitness = stats['Return [%]'] * stats['SQN']
        fitness = (stats['Return [%]'] * stats['SQN']) / (abs(stats['Max. Drawdown [%]']) ** 0.25)
        # fitness = stats['Return [%]'] * stats['Kelly Criterion']
        # fitness = stats['Return [%]'] * stats['Profit Factor']
        ...
        # print(f"백테스팅 중 오류 발생: {e}") # 디버깅 시 주석 해제
        return 0.0,
...
def init_deap_creator_MOO(): # MOO = Multi-Objective Optimization
    """
    Sortino(Max)와 Calmar(Max)를 위한 
    다중 목표 최적화용 creator를 초기화합니다.
    """
...
def ts_optimization(train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame):
    ...
    2. [수정] 워밍업 데이터를 포함하여 train/val/test 신호를 생성합니다.
    """
    
    # --- [0단계] 워밍업 데이터 결합 ---
    # Train은 이전 데이터가 없으므로 자체 사용
    ...
    print("     - 이동평균(MA) 파라미터 최적화 (순수 Train 데이터 사용)...")
    ...
    # 2) 계산된 컬럼만 뽑아서 reindex 후 원본 DF에 병합
    ...
    print("     - 모든 기술적 지표 최적화 및 신호/Slope 생성 완료. (데이터 소실 방지 적용)")
...
def esn_rolling_forward(df: pd.DataFrame, n_splits: int = 5, initial_train_ratio: float = 0.5,
    ...
    # --- [수정됨] 0단계: 전체 데이터에 대해 CPM 정답 생성 (데이터 분할 전) ---
    # 분할된 구간의 경계면에서 파동이 끊기는(Boundary Artifact) 현상을 방지하기 위함입니다.
    print(f"--- 전체 데이터에 대해 CPM 정답지 생성 중 (P=0.05, T=5) ---")
    _, df_with_cpm = cpm.cpm_model(df, column='Close', P=0.05, T=5)

    # CPM 라벨이 포함된 데이터프레임을 분할합니다.
    ...
    for i, (train_df, validation_df, test_df) in enumerate(splits):
        ...
        try:
            ...
            # GA가 보지 못한 Test 데이터로 최종 백테스트
            print(f"[{i+1}/{n_splits}] 3b. ESN 최종 평가 (Test 데이터)...")
            stats, _ = perform_final_backtest(
                train_df=final_train_signals,     # ⬅️ Train 신호 데이터
                test_df=final_test_signals,     # ⬅️ Test 신호 데이터
                best_params=best_params,        # ⬅️ 2단계에서 찾은 최적 파라미터
